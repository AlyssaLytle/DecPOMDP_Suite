...
import os
import itertools
from MADPtools.DPOMDP_Writer.DPOMDPWriterMedium import DPOMDPWriterACC
import sys
sys.path.append("/DPOMDP_Writer")
from MADPtools.DPOMDP_Writer.Reader import *
import csv
from MADPtools.PrintMethods import *
...
for scenario in range(1,9):
    for start_state in modes:
        ...
        rel_path = "MADPtools/ACC/" + filename + ".dpomdp"
        abs_file_path = os.path.join(script_dir, rel_path)
        writer.write_to_file(abs_file_path, start_state)


### Generate shell file to run all of these ###
shell_file = "MADPtools/ACC.sh"
file_data = ""
for scenario in range(1,9):
    for start_state in modes:
        filename = "ACC-ss-" + start_state + "-scen-" + str(scenario)
        rel_path = "MADPtools/ACC/" + filename + ".dpomdp"
        #command = "../MADP/src/solvers/GMAA --GMAA=MAAstar --BGIP_Solver=BnB --BnB-ordering=Prob " + rel_path + " -h2"
        command = "~/public/alyssadecpomdp/MADP/src/solvers/GMAA --GMAA=kGMAA --BGIP_Solver=AM " + rel_path + " -h2"
        file_data += command + "\n"
f = open(shell_file, "w")
...
shell_file = "MADPtools/GetResults.sh"
file_data = ""
for scenario in range(1,9):
    for start_state in modes:
        filename = "ACC-ss-" + start_state + "-scen-" + str(scenario)
